...
import {
  Card, CardHeader, Button, Modal, Input, Select, Textarea,
  Badge, Table, Th, Td, EmptyState, StockBar
} from "@/components/ui";
...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({
    ...
    price: "", stock: "", barcode: "", lowStockThreshold: "10",
  });
  ...
  const openAdd = () => {
    setForm({ name: "", description: "", category: "Electronics", price: "", stock: "", barcode: "", lowStockThreshold: "10" });
    setEditing(null);
    setShowModal(true);
  };
  ...
  const openEdit = (p: ProductType) => {
    setForm({
      ...
      price: String(p.price), stock: String(p.stock), barcode: p.barcode || "",
  ...
  const stockBadge = (p: ProductType) => {
    ...
    if (p.stock <= p.lowStockThreshold) return <Badge variant="orange">Low Stock</Badge>;
  ...
  return (
    <AppShell title="Product Management">
      ...
      <Card>
        <CardHeader>
          <div>
            <div className="font-bold text-white text-sm">Product Inventory</div>
            <div className="text-xs text-gray-600 mt-0.5">{products.length} products</div>
          </div>
          <Button variant="secondary" size="sm">⬇ Export CSV</Button>
        </CardHeader>
        {loading ? (
          <div className="py-16 text-center text-gray-600 animate-pulse">Loading products…</div>
        ) : (
          <Table>
            <thead>
              <tr>
                <Th>Product</Th><Th>Category</Th><Th>Price</Th>
                <Th>Stock</Th><Th>Barcode</Th><Th>Status</Th><Th>Actions</Th>
              </tr>
            </thead>
            <tbody>
              {products.length === 0 ? (
                ...
                <tr key={p.id} className="hover:bg-white/[0.02]">
                  ...
                  <Td><span className="text-orange-400 font-bold">${Number(p.price).toFixed(2)}</span></Td>
                  <Td><StockBar stock={p.stock} threshold={p.lowStockThreshold} /></Td>
                  <Td><span className="font-mono text-xs text-gray-600">{p.barcode}</span></Td>
                  <Td>{stockBadge(p)}</Td>
                  <Td>
                    <div className="flex gap-2">
                      <Button variant="secondary" size="sm" onClick={() => openEdit(p)}>✏️</Button>
                      <Button variant="danger" size="sm" onClick={() => setConfirmDelete(p)}>🗑️</Button>
                    </div>
                  </Td>
                </tr>
              ))}
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <Textarea label="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Brief product description" />
          </div>
          <Select
            label="Category"
            value={form.category}
          ...
          <Input label="Low Stock Threshold" type="number" value={form.lowStockThreshold} onChange={(e) => setForm({ ...form, lowStockThreshold: e.target.value })} placeholder="10" />
          <div className="col-span-2">
