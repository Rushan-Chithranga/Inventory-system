...
export default function UsersPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", password: "", role: "Store Manager" });
  ...
  return (
    <AppShell title="Users & Roles">
      ...
      <div className="flex items-center justify-end mb-5">
        <Button onClick={() => { setShowModal(true); setError(""); }}>+ Invite User</Button>
      </div>

      <Card className="mb-5">
        <CardHeader>
          <div className="font-bold text-white text-sm">Users & Access Control</div>
        </CardHeader>
        {loading ? (
          ...
          <Table>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan={6}><EmptyState icon="👥" message="No users found" /></td></tr>
              ) : users.map((u) => {
                    ...
                    <Td>
                      <div className="flex items-center gap-2.5">
                        <div
                          ...
                          style={{ background: u.role === "Admin" ? "linear-gradient(135deg,#f97316,#ea580c)" : "linear-gradient(135deg,#3b82f6,#2563eb)" }}
                        >
                          {initials}
                        </div>
                        <div>
                          <div className="font-semibold text-white">{u.name}</div>
                          {isMe && <div className="text-[10px] text-orange-500">(You)</div>}
                        </div>
                      </div>
                    </Td>
                    <Td><span className="text-gray-500">{u.email}</span></Td>
                    <Td>
                      <span className={u.role === "Admin" ? "text-orange-500 font-semibold" : "text-blue-400 font-semibold"}>
                        {u.role === "Admin" ? "🔑 Admin" : "🏪 Store Manager"}
                      </span>
                    </Td>
                    <Td><Badge variant={u.isActive ? "green" : "gray"}>{u.isActive ? "Active" : "Inactive"}</Badge></Td>
                    <Td><span className="text-xs text-gray-600">{u.lastLogin ? new Date(u.lastLogin).toLocaleDateString() : "Never"}</span></Td>
                    <Td>
                      <div className="flex gap-2">
                        <Button variant="secondary" size="sm">✏️ Edit</Button>
                        {!isMe && <Button variant="danger" size="sm">Remove</Button>}
                      </div>
                    </Td>
                  </tr>
                );
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <div className="text-xs text-gray-600 mt-0.5">Access control matrix</div>
          </div>
        </CardHeader>
        <Table>
          <thead><tr><Th>Feature</Th><Th>🔑 Admin</Th><Th>🏪 Store Manager</Th></tr></thead>
          <tbody>
            {PERMISSIONS.map(([feat, admin, mgr]) => (
              <tr key={feat as string} className="hover:bg-white/[0.02]">
                <Td><span className="text-gray-300">{feat as string}</span></Td>
                <Td><span className={admin ? "text-green-400 font-bold" : "text-red-400 font-bold"}>{admin ? "✓ Yes" : "✕ No"}</span></Td>
                <Td><span className={mgr ? "text-green-400 font-bold" : "text-red-400 font-bold"}>{mgr ? "✓ Yes" : "✕ No"}</span></Td>
              </tr>
            ))}
          </tbody>
        </Table>
