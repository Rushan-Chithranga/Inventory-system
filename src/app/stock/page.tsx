...
export default function StockPage() {
  ...
  const fetchProducts = () => {
    ...
    if (filter !== "all")
      params.set(
        "status",
        filter === "low" ? "low" : filter === "out" ? "out" : "ok",
      );
    setLoading(true);
    ...
    <AppShell title="Stock Report">
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          icon={PackageIcon}
          changeType="warn"
          change={lowCount > 0 ? "Needs restocking" : ""}
        />
        <StatCard
          label="Out of Stock"
          value={outCount}
          icon={PackageOutOfStockIcon}
          changeType={outCount > 0 ? "down" : undefined}
          change={outCount > 0 ? "Urgent reorder!" : ""}
        />
        <StatCard
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={applyAdjust} disabled={saving || !adjQty}>
              {saving ? "Saving…" : "Apply"}
            </Button>
          </>
        }
      >
        ...
        {adjustModal && (
          <div className="mt-3 bg-[#0d0d0d] rounded-lg px-4 py-3 text-sm text-gray-500">
            Current:{" "}
            <span className="text-white font-bold">{adjustModal.stock}</span>
            {adjQty && (
              <>
                {" "}
                → New:{" "}
                <span className="text-orange-400 font-bold">
                  {adjType === "add"
                    ? adjustModal.stock + parseInt(adjQty || "0")
                    : Math.max(0, adjustModal.stock - parseInt(adjQty || "0"))}
                </span>
              </>
            )}
          </div>
