...
export default function SalesPage() {
  ...
  useEffect(() => { fetchSales(); }, [search, category, dateFrom, dateTo]);
  ...
  return (
    <AppShell title="Sales Report">
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        >
          {CATEGORIES.map((c) => (
            <option key={c} value={c} className="bg-[#1a1a1a]">{c || "All Categories"}</option>
          ))}
        </select>
        <input
          type="date"
        ...
        {(search || category || dateFrom || dateTo) && (
          <Button variant="ghost" size="sm" onClick={() => { setSearch(""); setCategory(""); setDateFrom(""); setDateTo(""); }}>
            Reset
          </Button>
        )}
      ...
      <Card>
        <CardHeader>
          <div>
            <div className="font-bold text-white text-sm">Sales Transactions</div>
            <div className="text-xs text-gray-600 mt-0.5">{sales.length} records</div>
          </div>
        </CardHeader>
        {loading ? (
            ...
            <tbody>
              {sales.length === 0 ? (
                ...
                <tr key={s.id} className="hover:bg-white/[0.02]">
                  ...
                  <Td><Badge>{s.productCategory}</Badge></Td>
                  <Td>×{s.quantity}</Td>
