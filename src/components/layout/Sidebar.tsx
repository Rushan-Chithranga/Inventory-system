...
const NAV: NavItem[] = [
  ...
  { href: "/barcode", label: "Barcode Scanner", icon: "▥" },
];

const REPORTS: NavItem[] = [
  { href: "/sales", label: "Sales Report", icon: "📊" },
  { href: "/stock", label: "Stock Report", icon: "🗄️" },
  ...
  return (
    <aside className="fixed top-0 left-0 bottom-0 w-60 bg-[#111] border-r border-[#1e1e1e] flex flex-col z-50">
      ...
      <div className="px-5 py-6 border-b border-[#1e1e1e] flex items-center gap-3">
        <div className="w-9 h-9 bg-orange-500 rounded-lg flex items-center justify-center text-lg flex-shrink-0">
          📦
        </div>
        <div>
      ...
      <nav className="flex-1 p-3 overflow-y-auto flex flex-col gap-0.5">
        ...
        {REPORTS.map((item) => (
          <NavLink
            ...
            badge={item.href === "/stock" && lowStockCount > 0 ? lowStockCount : undefined}
          />
        ))}
